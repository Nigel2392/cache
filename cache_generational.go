...
/*
	...
	func GetUser(ctx context.Context, userID string) (User, error) {
	    ...
	    key, err := GenerationKey(ctx, "users", userID)
	    if err != nil {
	        return User{}, err
	    }
	    ...
	    _ = Set(ctx, key, user, time.Minute*15)
	...
	func InvalidateAllUsers(ctx context.Context) error {
	    ...
	    _, err := NextGeneration(ctx, "users", time.Hour*24)
	    return err
	}
	...
	func SeedNewProducts(ctx context.Context, products []Product) error {
	    ...
	    genKeyFunc, err := NextGeneration(ctx, "products", 0)
	    if err != nil {
	        return err
	    }
	    ...
	    for _, p := range products {
	        ...
	        _ = Set(ctx, genKeyFunc(p.ID), p, time.Hour)
	    }
	    return nil
	}
	...
	func GetCustomData(ctx context.Context, myRedis Cache, itemID string) {
	    // Use the ...FromCache variants to bypass Default()
	    key, _ := GenerationKeyFromCache(ctx, myRedis, "custom_group", itemID)
